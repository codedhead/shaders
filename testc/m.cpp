...
float pdfA2W(float pdf_area,vec3 p,vec3 p_next,vec3 n_next)
{
	...
	float dist2=dot(w,w);
	w*=sqrt(dist2);
	return pdf_area*dist2 /**safe_rcp*/ / (abs(dot(n_next,w)));
}
...
vec2 iplane_size=2.*tan(0.5*cam_vfov)*make_float2(resolution.x/resolution.y,1.);
...
{
	vec2 ixy=(pixel/resolution.xy - 0.5)*iplane_size;
...
vec3 shade(float objId,vec3 p,vec3 n,Ray ray,int depth,vec3 alpha,OUT_PARAM(vec3) f)
{
	f=make_float3(0.);
	if(objId==0.)
	{
	...
	if(objId==OBJ_LIGHTf)
	{
		...
		if(depth>1)
		{
			...
			float bsdf_pdf=bsdfPdf(p-ray.origin,n);//INV_2PI;
	...
	else
	{
		...
		if(!shadowed)
		{
			...
			// light is not delta, do mis
			float bsdf_pdf=bsdfPdf(Lp-p,Ln);
			float lgt_pdf=pdfA2W(pdf_a,p,Lp,Ln);
			float wgt=(lgt_pdf*lgt_pdf)/(lgt_pdf*lgt_pdf+bsdf_pdf*bsdf_pdf);						

			res=wgt*alpha*f*INV_PI*g*L/pdf_a;
		}
	}
